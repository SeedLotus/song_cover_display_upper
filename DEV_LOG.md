...
- **2026-09-25 实机日志修正**：在未烧录修复固件的 v1 设备上 `/a` 为 **100% 丢失**（7/7 次传输均超时），并非偶发；封面均实际显示成功。v1 下任何逻辑都不得依赖 `/a` 到达（2.34 已将转动恢复改挂"收到 /a 或 AwaitingAck 超时"双触发）。
...
- **多次刷新**：2026-09-25 实机确认为网易云播放/暂停推送同封面变体触发重传，2.32 感知哈希去重修复（纯上位机），实机待验证。
- **验证清单（v1 固件 + 新上位机）**：网易云播放/暂停不再出现下滑动画与停转；切歌封面正常更新；同专辑连播不重复传输；真实切歌后转动停止为 v1 已知限制（暂停/播放可恢复），烧录 v2 固件后由 2.31 机制彻底解决。
...
### 2.32 感知哈希去重：修复网易云播放/暂停触发封面重传（2026-09-25，纯上位机方案）

- **现象**：v1.2.0-dev 实机反馈——网易云播放/暂停仍然停转，且播放和暂停都会播一次封面下滑动画。
- **根因**：网易云在播放/暂停状态变化时推送**同一封面的不同分辨率/编码变体**；`ComputeImageHash` 是原始像素 MD5，变体像素不同 → hash 不同 → 确认阶段（forceHashCheck，2.26 引入时移除了同身份捷径）触发完整传输 → `/t` 停转 + `/o` 下滑动画。v1 固件下传输后无法安全恢复转动（全固件仅 `/1` 能开转，而设备实际固件的 `/1` 语义无法在线验证：fork 源码摇臂脚本是目标导向状态机=设态语义，与 2.18 实机记录的翻转语义矛盾），表现为停转。
- **修复**：`ImageProcessor` 新增 8x8 灰度 aHash 感知哈希（64 位）与汉明距离；`ProcessAlbumArtAsync` 在 MD5 不同后追加感知比对，距离 ≤8/64 判定为同一封面变体（含同专辑不同曲目）→ 跳过重传，仅更新媒体身份，感知基准保持为下位机实际显示的封面。
- **效果**：播放/暂停不再产生任何封面传输——无下滑动画、无 `/t` 停转；播放时 `/1` topic 自然恢复转动。同专辑连播（封面相同）也不再重复传输。
- **v1 固件残留限制**：真实切歌（封面确实变化）的传输仍会停转且 v1 下无安全恢复手段，暂停/播放一次可手动恢复；彻底修复依赖 v2 固件（2.31 已实现，待烧录）。
- **固件约束**：本轮纯上位机修改，固件仓库未动。

### 2.33 固件语义主动探测：v1 下真实切歌后自动恢复转动（2026-09-25，纯上位机方案）

- **背景**：2.32 后播放/暂停已无碍，但真实切歌（封面必须传输）后 v1 固件仍停转。恢复转动只能发 `/1`，而设备实际固件对重复 `/1` 是设态（安全）还是翻转（摇臂反向）无法在线验证——fork 源码（目标导向状态机=设态）与 2.18 实机记录（翻转）矛盾。
- **探测原理**：believed=暂停（摇臂抬起）时发一个**重复 `/0`**——设态固件对暂停态的暂停指令无反应；翻转固件会把状态翻成播放，摇臂开始放下（肉眼可见，且暂停时不转，现象唯一）。用户观察 10 秒回答二值结果。
- **实现**：
  - 主界面新增「检测转动恢复」按钮与结果文本；探测前提：已连接、无传输中、`_lowerMachinePlaying=false`（防误判），引导用户保持暂停并观察。
  - 结果持久化 `settings.json` 的 `FirmwareSemantics`（Unknown/SetState/Toggle），一次探测永久有效。翻转结果自动再发 `/0` 翻回暂停恢复摇臂。
  - `HandleImageTransferAck` 无条件补发条件从"v2 协商"扩展为"v2 协商 **或** 探测为 SetState"——探测通过的设备，真实切歌传输完成 1s 后自动补 `/1` 恢复转动，无需烧录固件。
- **固件约束**：纯上位机修改，固件仓库未动。

### 2.34 实机日志定案：/a 必丢覆盖 + 播放指令合并防抖（2026-09-25）

- **实机日志三个决定性事实**（首次启用 2.30 文件日志后分析）：
  1. **`/a` 在 v1 设备上 100% 丢失**（7 次传输全部 AwaitingAck 超时，封面实际均显示成功）——3.4 在这台设备上不是偶发是必现，即固件 `7328afd` 修复的 TX-busy 丢 `/a` 问题的未烧录表现。2.33 挂在"收到 /a"上的转动恢复因此从未执行（用户反馈"启用设态后依然没有自动恢复"的根因）。
  2. **语义探测结果可信**：探测 /0 摇臂无动作，设备确为设态固件，2.18"翻转"结论系误诊。
  3. **"误动作"根因**：网易云每次播放/暂停都重建 SMTC 会话，新会话初始化瞬间报出过时状态，造成 Paused→Playing→Paused 的 50ms 级抖动（日志实锤 50ms 内 /0 /1 /0 三连），每次抖动=一对摇臂指令→摇臂抽动。这也是 2.18"摇臂错乱"的真正来源。
- **修复**：
  - `ScheduleRotationRestoreAfterTransfer` 提取为独立方法，除"收到 /a"外，**AwaitingAck 超时也触发**（此时下位机多半已显示封面并播完动画，/t 停转同样需要恢复）；并增加接力传输顺延（最多约 3 秒），避免 /1 插进图片包流。
  - 播放指令 **200ms 合并防抖**：`OnPlaybackStateChanged` 的同步改为延迟 200ms 发最终稳定状态，消除会话重建抖动产生的多余 topic 对（摇臂误动作）。UI 状态显示仍即时更新。
