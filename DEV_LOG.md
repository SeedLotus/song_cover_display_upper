...
### 3.8 持续播放一段时间后屏幕变暗，暂停或切歌才恢复亮起（v1.2.0 用户反馈，待修复）

- **现象（2026-09-25 用户实机反馈）**：播放过程中超过一定时间屏幕暗掉；暂停或切歌后重新亮起。
- **根因判断（高置信，下次迭代验证）**：下位机 60 秒无"有效命令"进入休眠（背光关闭）。v1 固件忽略 `/h` 心跳（3.6 的 `/h` 是未烧录的新固件特性），而 2.18 起上位机状态一致时只发 `/h`——持续播放场景下 v1 设备实际收不到任何能重置休眠计时的命令，60 秒后休眠变暗。暂停/切歌时的 `/0` `/t` 会调用 `_usb_get_up()` 重置计时并恢复背光，与现象吻合。
- **候选修复（纯上位机）**：探测为设态语义（2.33）的设备上，周期性（如 30s）补发一次当前播放状态指令充当 v1 保活——设态固件对重复 `/1` `/0` 无副作用。注意实测 `/1` 附带 `disp_pic_rotate(1)` 重初始化旋转脚本是否有可见闪烁；有则仅暂停态用 `/0` 保活、播放态接受休眠或降低频率。
- **彻底修复**：烧录 v2 固件后 `/h` 生效，本问题自然消失。
- **下次迭代时提醒用户本问题**。
