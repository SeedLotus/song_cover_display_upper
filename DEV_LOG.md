...
### 3.7 网易云音乐：切换/暂停歌曲导致封面多次刷新且封面不再转动（v1.1.0 用户反馈，待修复）

- **现象（2026-09-24 用户实机反馈，v1.1.0 包）**：使用网易云音乐播放时，切换歌曲和暂停歌曲会导致封面**多次刷新**；并且封面**不再转动**。
- **初步排查方向（下次开发时优先）**：
  - 网易云 SMTC 会话在切歌/暂停时可能短时间内连续触发多次属性变更事件（与 2.25 B站事件滞后类似但触发源不同），现有防抖（120ms+500ms 两段式拉取）与媒体身份去重（`_lastMediaTitle/Artist/Album`）可能对网易云的事件序列不生效，需在实机上加日志确认事件次数与时间间隔。
  - "封面不再转动"需先确认指代：下位机转盘停止旋转（可能播放状态 topic 未下发或下位机休眠），还是上位机 UI 效果。若与 3.6 心跳机制相关，检查 `/h` 是否在状态一致时被正常发出、网易云暂停时 believed state 是否卡在不一致态。
  - 对比验证：同一台机器用 PotPlayer / 其他播放器复现，确认是否为网易云特有。
- **状态**：未修复，下次优化开发时提醒用户并优先处理。
