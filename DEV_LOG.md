...
### 3.7 网易云音乐：切换/暂停歌曲导致封面多次刷新且封面不再转动（v1.1.0 用户反馈）—— 已闭环（v1.2.0）
...
- **闭环记录（2026-09-25）**：2.32 感知哈希去重 + 2.33 语义探测转动恢复 + 2.34 /a 必丢双触发与会话抖动防抖 + 2.35 延迟优化 + 2.36 便携设置首跑自助探测，v1.2.0 实机验收全部通过。

### 3.8 持续播放一段时间后屏幕变暗，暂停或切歌才恢复亮起（v1.2.0 用户反馈）—— 已修复（待实机验证）

- **现象（2026-09-25 用户实机反馈）**：播放过程中超过一定时间屏幕暗掉；暂停或切歌后重新亮起。
- **根因（固件源码证实）**：下位机 60 秒无"有效命令"进入休眠（背光关闭，`SLEEP_TICKS_COUNT = 60*1000`）。v1 固件忽略 `/h` 心跳（`/h` 处理是未烧录的 7328afd 新固件特性），而 2.18 起上位机状态一致时只发 `/h`——持续播放场景下 v1 设备实际收不到任何能重置休眠计时的命令，60 秒后休眠变暗。暂停/切歌时的 `/0` `/t` 会调用 `_usb_get_up()` 重置计时并恢复背光，与现象吻合。
- **修复（2026-09-26，纯上位机）**：`StatusSendTimer_Tick` 状态一致分支改为分级保活——
  - v2 固件（`/v` 协商成功）：维持每秒 `/h`（有效且零副作用）；
  - v1 + 探测为设态语义：每 25s 补发一次当前播放状态指令（`/1` 或 `/0`）充当保活。v1 下 `/1` `/0` 均调用 `_usb_get_up()` 重置休眠计时；设态语义下重复发送对摇臂无副作用。25s 间隔保证单包丢失最坏 50s < 60s 阈值。不更新 `_lastPlaybackTopicSentAt`、不动 believed state（状态本来就一致）；
  - v1 翻转/未探测设备：维持 `/h`（被忽略，休眠无法安全避免），需烧录 v2 固件。
- **已知代价（待实机观察）**：播放中 `/1` 保活会执行 `disp_pic_rotate(1)` → `HAL_SPI_DMAStop` + 旋转脚本重启，`disp_angle` 不重置但可能每 25s 出现一次单帧级轻微闪烁。若肉眼可见且不可接受，选项：拉长间隔（牺牲丢包余量）或烧录 v2 固件（`/h` 零副作用，彻底修复）。
- **验证要点**：持续播放 >60s 屏幕不再变暗；观察每 25s 有无可见闪烁；日志应有 `v1 保活：补发当前状态` 每 25s 一条。

### 3.9 电脑上快速连续播放/暂停切换时，反向控制（下位机→上位机）生效错位（2026-09-25 用户反馈）—— 已修复（待实机验证）

- **现象**：在电脑上迅速连续播放/暂停切换时，下位机上行的播放/暂停请求（`/q0` `/q1`）作用于已过期的状态，把用户最新操作反转回去。
- **根因（固件源码证实）**：下位机摇臂状态机在稳定态（暂停态 case 5 / 播放态 case 8）每 ~123ms 轮询摇臂物理位置，位置与软件状态不一致即上行 `/q1`（暂停态检测到靠近）/ `/q0`（播放态检测到离开），且持续不一致时周期重发。摇臂物理动作滞后（预备态 duty 渐变驱动，最长约 2s）：PC 侧快速切换期间，摇臂还在追上一个指令，稳定态轮询会读到"过期的不一致"并上行 /q；上位机原实现无条件应用（`TryPlayAsync`/`TryPauseAsync`），等于把用户刚做的切换反转。
- **修复（2026-09-26，纯上位机）**：新增 `IsReverseControlSuppress`——距上次下发播放 topic（`_lastPlaybackTopicSentAt`）2.5s 内的 `/q0` `/q1` 一律丢弃并记日志。快速切换期间每次真实切换都会刷新该时间戳，整个风暴窗口内反向控制全部抑制；风暴平息 2.5s 后恢复正常。
- **自愈保证**：下位机在摇臂持续不一致时每 ~123ms 重发 `/q`，若用户在窗口内真实手动拨臂，窗口过期后的重发仍会生效，不会永久丢失反向控制。
- **附带收益**：手动拨臂后上位机应用反向控制 → SMTC 事件触发同步下发 topic → 刷新 `_lastPlaybackTopicSentAt` → 下位机随后的重复 `/q` 自动被抑制，消除了重复 `/q` 导致的冗余 `TryPauseAsync`/重复 topic。
- **验证要点**：电脑上快速连点播放/暂停（间隔 <1s 多次），最终状态应与最后一次操作一致，不被反转；静置 3s 后手动拨臂，反向控制仍生效；日志应有 `抑制反向控制 /qX` 记录。
...
### 2.35 全链路延迟优化（2026-09-25，验收后推送）

- **背景**：2.34 修复转动恢复后，实机确认恢复生效，但全链路存在多处较长等待。
- **优化项**（基于实机日志的延迟拆解）：
  1. **摇臂响应**：200ms 一刀切防抖改为自适应——距上次发 topic >500ms 视为真实操作立即发送（0 延迟）；500ms 窗口内的连续变化按抖动 150ms 合并。播放/暂停摇臂恢复瞬时响应，抖动仍被吸收。
  2. **封面切换**（优化前事件→/o 约 980ms）：`/k` 等待从 2×150ms+200ms（500ms）砍到 1×100ms+100ms（200ms）——v1 设备 `/k` 回复与 `/a` 同源 100% 丢失，长等待纯属浪费；重连场景仍用 12 次长重试。优化后约 550ms（120ms SMTC 防抖不能动，防 2.25 旧封面回归）。
  3. **旋转恢复**（优化前 /o 后约 2s：`/a` 超时 1000ms 白等 + 恢复延迟 1000ms）：改为 `/o` 一发出即调度恢复，延迟 700ms 刚好覆盖下落动画；`/a` 收到与超时仅作兜底，三个触发点按 transferId 去重。旋转在切歌后约 1.2s 恢复。AwaitingAck 超时 1000→600ms（仅状态机复位用）。

### 2.36 探测按钮按结果显隐 + 便携设置陷阱（2026-09-25，实机反馈修复）

- **现象**：v1.2.0 打包版（publish 目录）运行后，切歌首次封面不旋转问题复现；Debug 目录运行正常。
- **根因**：`settings.json` 便携模式跟随 exe 目录。Debug 目录有探测结果 `SetState`，打包版目录为全新环境（Unknown）→ 转动恢复按安全设计不启用；而 2.33 验收后探测按钮被固定隐藏，打包版无法探测。打包版日志证实传输超时后无补发记录。
- **修复**：探测按钮改为仅在 `FirmwareSemantics == "Unknown"` 时显示（`UpdateFirmwareSemanticsText` 统一控制），新部署目录首跑可见、探测后自动隐藏。
- **教训**：便携模式下"探测结果/设置"是按部署目录隔离的，任何依赖本地探测/设置的功能在新目录首跑必须可自助完成初始化。
- **发布流程固化**：Release 产物启用 `EnableCompressionInSingleFile`（179.5MB → 74.8MB，-58%），v1.2.0 Release 资产已替换为压缩版。
...
| 2026-09-26 | 3.8 v1 保活 + 3.9 反向控制抑制 | 编译通过 | dotnet build 0 警告 0 错误；纯上位机改动，实机待验证（3.8 观察 25s 周期闪烁，3.9 快速切换+手动拨臂） |
